...
int findSurfer(unsigned char surfer){
  for(int i = 0; i < 8; i++){
    if((surfer >> i) & 1) return 7 - i;
  }
  return -1;
}

int findHole(char train){
  for(int i = 0; i < 8; i++){
    if(((train >> i) & 1) == 0) return 7 - i;
  }
  return -1;
}
...
int *planEscape(char trains[], unsigned char surfer){
    ...
    int surferPos = findSurfer(surfer);
    //printf("surferPos = %d\n", surferPos);
    int hole = 0;
    for(int i = 0; i < 5; i++){
        hole = findHole(trains[i]);
        //printf("hole = %d\n", hole);
        returnValue[i] = hole - surferPos;
        surferPos = hole;
    }
