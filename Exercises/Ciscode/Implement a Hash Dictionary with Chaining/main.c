...
void insertUniqueSorted(Dictionary D, int elem){
    int hashKey = hash(D, elem);
    SET *trav = &D[hashKey], newNode, temp;
    
    for(temp = D[hashKey]; temp != NULL; temp = temp->next){
        if(temp->data == elem){
            printf("Element 30 is already in the dictionary.\n");
            return;
        }
    }
